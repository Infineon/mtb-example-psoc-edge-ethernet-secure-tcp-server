...
The following example clones the "[Ethernet Secure TCP server](https://github.com/Infineon/mtb-example-psoc-edge-ethernet-secure-tcp-server)" application with the desired name "EthernetSecureTcpServer" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-ethernet-secure-tcp-server --user-app-name EthernetSecureTcpServer --target-dir "C:/mtb_projects"
   ```
